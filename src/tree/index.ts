...
export class BinaryTree extends Tree {
    ...
    private static _inOrderTraversal<D>(node: BinaryTreeNode<D> | null, output: D[]): D[] {
        if (node !== null) {
    ...
    private static _preOrderTraversal<D>(node: BinaryTreeNode<D> | null, output: D[]): D[] {
        if (node !== null) {
            const [left = null, right = null] = node.children;
            output.push(node.value);
    ...
    private static _postOrderTraversal<D>(node: BinaryTreeNode<D> | null, output: D[]): D[] {
        if (node !== null) {
            const [left = null, right = null] = node.children;
...
export class Heap<D> {
    ...
    constructor(
        initializer?: D,
        comparator: Comparator<D> = (x, y): 1 | 0 | -1 => x < y ? -1 : x > y ? 1 : 0
    ) {
    ...
    append(value: D): number {
        ...
        while (parentIndex >= 0) {
            const comparison = this._comparator(this._nodes[childIndex], this._nodes[parentIndex]);
            switch (comparison) {
                case 1: return this._nodes.length;
                case -1:
    ...
    extractRoot(): D | undefined {
        ...
        while (!childrenIndices.every(c => this._nodes[c] === undefined)) {
            ...
            if (this._nodes[l] === undefined) childIndexToSwap = f;
                ...
                const childComparison = this._comparator(this._nodes[f], this._nodes[l]);
