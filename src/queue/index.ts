...
class QueueNode<D> {
  ...
  queue: Queue<D>;
  constructor(data: D, queue: Queue<D>) {
    this._data = data;
    this.queue = queue;
  }
  ...
  get next(): QueueNode<D> | null {
    return this._next;
  }

  set next(node: QueueNode<D> | null) {
    this._next = node;
  }
}
...
export class Queue<D> {
  ...
  enqueue(data: D): void {
    const node = new QueueNode<D>(data, this);
...
export class QueueStack<D> extends Queue<D> {
  ...
  peek(): D | null {
    return this._next ? this._next.data : null;
  }

  enqueue(data: D): void {
    if (this._next === null) this._next = new QueueNode<D>(data, this);
